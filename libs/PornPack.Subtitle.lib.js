...
window.PornSubtitle = class PornSubtitle {
    ...
    static renderCardIndicator(card, hasSubtitle) {
        ...
        const host = card.querySelector('.grid.grid-flow-col.justify-between');
        if (!host) return;
        ...
        // 直接沿用同一行厂牌图标的实际尺寸，避免 Tailwind 尺寸调整后两者失配。
        const makerIcon = Array.from(host.querySelectorAll('img, svg')).find(node => !indicator.contains(node));
        const size = makerIcon?.getBoundingClientRect?.();
